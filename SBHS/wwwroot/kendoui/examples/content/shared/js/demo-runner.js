        ...
        if (!target.hasClass(SWITCH_SELECTED_OP)) {
            ...
            if (target.text() == "TagHelper") {
                ...
                if (typeof (Storage) !== "undefined") {
                    ...
                    document.cookie = "isTagHelper=true; SameSite=Strict; Path=/; Secure";
                }
            }
            else {
                htmlHelper.show();
                ...
                if (typeof (Storage) !== "undefined") {
                    ...
                    document.cookie = "isTagHelper=false; SameSite=Strict; Path=/; Secure";
                }
            }
        }

        toggleApiButton(target.text());
    }

    function toggleApiButton(target) {
        var serverApiButtonText = $(".server-api:visible").text().replace(/\s/g, "").toLowerCase();
        if (serverApiButtonText.indexOf(target.toLowerCase()) == -1) {
            $(".server-api").toggle();
        }
    }

    function addCopyButton(element) {
        $(element).prepend('<span class="copy-code-btn k-icon k-font-icon k-input-icon k-i-copy" style="float:right; cursor: pointer;" title="Copy Code"></span>');
